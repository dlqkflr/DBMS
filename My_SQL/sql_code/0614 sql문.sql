...
-- 값이 참이라면 첫번째, 거짓이면 두번째
SELECT IFNULL(NULL, '널이군요'), IFNULL(100, '널이군요');
-- 값이 null이라면 뒤에값이 나오고 아니면 그냥 값이 나옴
SELECT NULLIF(100,100), NULLif(200,100);
-- 두개가 같으면 null 같지 않으면 앞에값이 나옴.
...
ELSE '모름'
END;
-- case 뒤에 값을 넣어서 when 조건을 거쳐서 값이 나옴.

SELECT BIT_LENGTH('abc'), CHAR_LENGTH('abc'), LENGTH('abc'); -- length : byte 길이
SELECT BIT_LENGTH('가나다'), CHAR_LENGTH('가나다'), LENGTH('가나다'); -- 한글은 한글자당 3바이트
...
-- 5자리로 만들어 주고 남는공간에 ##을 채워라 (왼쪽, 오른쪽)
...
SELECT U.userID, U.name, B.prodName, U.addr, U.mobile1 + U.mobile2 AS '연락처' -- 회원테이블기준
FROM userTbl U
INNER JOIN buyTbl B
