...
-- test table 생성    // test column 생성 - datatype -  DECIMAL(5,3)
create table test ( testcl decimal(5,3));
...
ALTER TABLE sales.test ADD COLUMN `test_f1` FLOAT(5,3) NULL AFTER `testcl`;


-- 값 입력 - test_fl : 14.55555 / test : 16.55555
insert into test (test_f1 , testcl) values (14.5555, 16.55555);
...
ALTER TABLE `sales`.`sales` CHANGE COLUMN `purchase_number` `purchase_number` INT(11) NOT NULL AUTO_INCREMENT ,
ADD PRIMARY KEY (`purchase_number`);

-- customers 테이블의 customer_id를 primary key로 지정

alter table `sales`.`customers` change column `customer_id` `customer_id` int not null , add primary key(`customer_id`);
