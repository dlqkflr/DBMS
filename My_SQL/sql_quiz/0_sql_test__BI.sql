...
alter table test add test_f1 float(5,3) null after test;




## 값 입력 - test_fl : 14.55555 / test : 16.55555
insert into test values(16.55555, 14.5555);
...
## sales table의 customer_id에 외래키 추가
## 참조 테이블 - customers
## 삭제 시 자동 없데이트 옵션
...
alter table customers add column gender 
	enum('m', 'f') after last_name;


## customers 테이블 값 추가 - '01', 'John', 'Mackinley', 'M', 'john.mckinley@365careers.com', 0
insert into customers 
values('01', 'John', 'Mackinley', 'M', 'john.mckinley@365careers.com', 0);
...
alter table companies 
 modify company_name varchar(200) not null;
...
select * from employees where first_name in ('Cathie', 
'Mark', 'Nathan');



## 이름이 'Cathie' , 'Mark', 'Nathan'에 해당하지 않는 row출력
select * from employees where first_name not in ('Cathie', 
'Mark', 'Nathan');
...
group by first_name having count(first_name) > 100;
