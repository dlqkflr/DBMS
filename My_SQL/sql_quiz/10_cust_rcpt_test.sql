...
SELECT * FROM rcpt_acct INNER JOIN cust_party ON rcpt_acct.ssn = cust_party.ssn;
SELECT * FROM rcpt_acct right outer join cust_party ON rcpt_acct.ssn = cust_party.ssn;

-- 1. 현재 살아있는 고객의 휴대폰 번호를 찾고, 살아있는 계좌수를 기준으로 오름차순 정렬하시오
select cust_party.MOBILE_NO, count(rcpt_acct.ACCT_NO) as acct_no from rcpt_acct 
    inner join cust_party on rcpt_acct.ssn = cust_party.SSN
    where rcpt_acct.CNCL_DT is null
    group by cust_party.MOBILE_NO
    order by acct_no;
    
-- 2. 현재 살아있는 계좌수가 두 개 이상이고 수신잔액의 총합이 50만원 이상인 고객의 주민번호, 이름, 휴대전화, 계좌수, 수신잔액 총합을 주민번호기준으로 오름차순 정렬하시오.
  
select cust_party.ssn, cust_party.PARTY_NM, cust_party.MOBILE_NO,
       count(rcpt_acct.ACCT_NO) as count_num, sum(rcpt_acct.RCPT_AMT) as sum_num
  from cust_party inner join rcpt_acct on rcpt_acct.ssn = cust_party.ssn
  group by cust_party.MOBILE_NO
  having count_num >=2 and sum_num >= 500000
  order by ssn;


select tmp1.ssn, tmp1.PARTY_NM , tmp1.MOBILE_NO, tmp2.cnt, tmp2.rcpt_amt from cust_party tmp1
left outer join 
(select ssn, count(*) as cnt, sum(rcpt_amt) as rcpt_amt from rcpt_acct where cncl_dt is null group by 1) tmp2
on tmp1.ssn = tmp2.ssn
where cnt >= 2 and RCPT_AMT >= 500000
order by tmp1.ssn;

select ssn, count(*) as cnt, sum(rcpt_amt) as rcpt_amt from rcpt_acct where cncl_dt is null group by 1;
