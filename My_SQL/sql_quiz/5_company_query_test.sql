...
select substring(residence_id, 7,1) 성별 from cust_info;
-- 2. 이메일을 보내기 위해 성과 이름을 결합하시오
select concat(first_nm, last_nm) from cust_info;
-- 3. 고객의 수익을 소수점 첫 째자리에서 반올림하시오. */
select first_nm,format(annl_perf,1) from cust_info;
...
select name,length(name) as name_len from vendor_info;
-- 4. 이름의 두 번째에서 네 번째 글자를 출력하고 name_str이라는 별칭으로 출력하시오
select name, substring(name, 2,3) name_str from vendor_info;
...
-- 2. 직원의 나이를 구하시오
select staff_nm ,2019-left(birth_dt,4) 나이 from clerk;
-- 3. 직원의 생일에 1달을 더한 날짜를 구하시오

-- 4. 남성의 평균 나이와 여성의 평균 나이를 구하시오
select gender, 2019-left(birth_dt,4) as 나이 from clerk group by gender;
