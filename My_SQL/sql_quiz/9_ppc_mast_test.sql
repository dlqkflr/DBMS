...
select sum(balance_amt), (case when acct_cd in(100, 110, 120, 130, 140) then 1
        else 0 end) as 수신구분 from ppc_mast_201312 group by 수신구분;
...
select ssn, sum(balance_amt) as 잔액합 from ppc_mast_201312 group by ssn having 잔액합 >= 2000000;

-- 4. 고객 당 금융상품 가입자수가 3개 이상인 고객을 구하시오


select ssn, count(acct_no) as sign_count from ppc_mast_201312 group by ssn having sign_count >= 3;
