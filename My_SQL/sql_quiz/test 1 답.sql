...
select cust_nm, cust_id, cust_birth, sales_amt from perf order by sales_cnt ;

-- 2. 고객 연령과 방문횟수를 비교하시오
select cust_birth, visit_cnt from perf order by cust_birth;

-- 3. 방문횟수가 가장 많은 고객의 이름을 출력하시오
select cust_nm, visit_cnt from perf order by visit_cnt desc limit 1 ;

-- 4. 1980년 이전 출생한 고객을 출력하시오.

select cust_nm, cust_birth from perf where cust_birth < '1980-01-01';

-- 5. 평균 방문횟수, 평균구입금액, 평균 구입상품 수를 구하시오

select avg(visit_cnt) as 평균방문횟수, avg(sales_amt) 평균구입금액 ,avg(sales_cnt) 평균구입상품수 from perf;

-- 6. 방문 당 구매금액이 가장 큰 고객의 이름을 구하시오
select cust_nm ,sales_amt /visit_cnt as '방문 당 구매금액' from perf order by sales_amt /visit_cnt desc limit 1;
